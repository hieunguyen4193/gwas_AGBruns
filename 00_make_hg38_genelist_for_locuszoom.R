...
selected.wd <- "/home/hieunguyen/CRC1382/src_2023/AGBruns_SNP_data_full/locuszoom_genelist_v38"
setwd(selected.wd)

gencode.v38 <- read.delim(file.path(selected.wd, "gencode.v38.annotation.gtf"), header=FALSE, comment.char="#", quote = "")

names(gencode.v38) <- c("chromosome_name", "annotation_source", "feature_type", "genomic_start", "genomic_end", "score", "genomic_strand", "genomic_phase", "tosplit")


#### convert final column into multiple columns (per annotation) ####
# add index column to track original row of info
gencode.v38$index <- 1:length(gencode.v38[, 1])

# split list of annotations - new annotation per row (into label & value as a single column)
gene_IDs <- gencode.v38 %>%
...
# original: write.table(gene_IDs, "~/Downloads/gencode.v38.txt", quote = FALSE, sep = "\t", na = "", row.names = FALSE, col.names = FALSE)
...
gencode.wide <- merge(gencode.v38[, -9], gene_IDs3, by = "index")

# save converted version of file
write.table(gencode.wide, file.path(selected.wd, "gencode.v38.wide.gtf"), quote=FALSE, sep = "\t", na = "", row.names = FALSE)
# original: write.table(gencode.wide, "~/Downloads/gencode.v38.wide.gtf", quote=FALSE, sep = "\t", na = "", row.names = FALSE)
...
ucsc.unique <- read.delim("/home/hieunguyen/CRC1382/src_2023/AGBruns_SNP_data_full/LocusZooms/UCSC_GRCh37_Genes_UniqueList2017.txt", header = TRUE, stringsAsFactors = FALSE)
...
write.table(gencode.wide.unique, "Gencode_GRCh38_Genes_UniqueListtxt", quote = FALSE, sep = "\t", na = "", row.names = FALSE)
