...
# source("/home/hieunguyen/CRC1382/src_2023/gwas_AGBruns/02_generate_Rdata_for_SNPStats.R")
...
path.to.main.src <- "/home/hieunguyen/CRC1382/src_2023/gwas_AGBruns"
path.to.project.src <- "/home/hieunguyen/CRC1382/src_2023/gwas_AGBruns"
all.GWAS.cases <- read.csv(file.path(path.to.project.src, "all_GWAS_cases.txt"), sep = "\t", header = FALSE)[["V1"]]

#####----------------------------------------------------------------------#####
##### BASIC ASSOC TEST RESULTS
#####----------------------------------------------------------------------#####
# path.to.Rmd.file <- "/home/hieunguyen/CRC1382/src_2023/gwas_AGBruns/03_generate_GWAS_reports.Rmd"
...
path.to.Rmd.file <- "/home/hieunguyen/CRC1382/src_2023/gwas_AGBruns/05_GWAS_with_models.Rmd"
for (disease.model in c("GENO", "TREND", "ALLELIC", "DOM", "REC")){
  for (maf in c(0.01, 0.05)){
    dir.create(file.path(path.to.save.HTML.dir, sprintf("GWAS_reports_disease_model_PLINK_maf_%s.addedQQplot", maf)), showWarnings = FALSE, recursive = TRUE)
    for (group in all.GWAS.cases){
      print("#####------------------------------------------------#####")
      print(sprintf("Working on %s, MAF: %s", group, maf))
      print("#####------------------------------------------------#####")
      ...
      dir.create(path.to.save.HTML.dir, showWarnings = FALSE)
      if (file.exists(file.path(path.to.save.HTML.dir, sprintf("GWAS_reports_disease_model_PLINK_maf_%s.addedQQplot", maf), save.HTML.filename)) == FALSE){
        rmarkdown::render(input = path.to.Rmd.file, 
                          params = list(
                            group = group,
                            maf = maf,
                          ...
                          output_file = save.HTML.filename,
                          output_dir = file.path(path.to.save.HTML.dir, sprintf("GWAS_reports_disease_model_PLINK_maf_%s.addedQQplot", maf))) 
      }
    }  
  }
}
...
writeLines(capture.output(sessionInfo()), file.path(path.to.main.src, "sessionInfo.txt"))  
