...
for group in ${groups};do \
    ...
    $plink --ped $path_to_ped_file --map $path_to_map_file --maf 0.05 --model --noweb --allow-no-sex --hwe 0.05

    mkdir -p ${outdir}/${PROJECT}/PLINK_results/maf_0.05_model/${group}_with_models_results_maf_0.05;

    mv plink.* ${outdir}/${PROJECT}/PLINK_results/maf_0.05_model/${group}_with_models_results_maf_0.05;done
